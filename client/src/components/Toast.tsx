...
function ToastNotification({ toast, onRemove }: ToastNotificationProps) {
  const getToastStyles = () => {
    const baseStyles =
      'max-w-sm w-full p-4 rounded-lg shadow-lg border flex items-start space-x-3 animate-in slide-in-from-right-full duration-300';
  ...
  const getIcon = () => {
    switch (toast.type) {
      ...
      case 'golden':
        return <span className='text-xl mt-0.5'>🏆</span>;
      case 'info':
      default:
        return (
          <svg
